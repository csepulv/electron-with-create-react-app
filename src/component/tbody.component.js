...
class Tbody extends Component {
  renderLine = consultation => {
    ...
    <tr key={`${consultation.id}-${consultation.date}`}>
      ...
      <td><a href='#' onClick={() => this.props.editConsultation(consultation.id)}><img className="icons" src={Edit} alt='edit'/></a></td>
      <td><a href='#' onClick={() => this.props.deleteConsultationAction(consultation.id)}><img className="icons" src={Remove} alt='remove'/></a></td>
    </tr>
  );
}
